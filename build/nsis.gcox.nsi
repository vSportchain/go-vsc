...
# 1. install gvsc for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for gvsc
# 5. create gvsc, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable VSPORTCHAIN_SOCKET
...
!define GROUPNAME "VSportChain"
!define APPNAME "Gvsc"
!define DESCRIPTION "Official Go implementation of the VSportChain protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since gvsc is installed global
