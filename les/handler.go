...
// NewProtocolManager returns a new vsportchain sub protocol manager. The VSportChain sub protocol manages peers capable
// with the vsportchain network.
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light VSportChain protocol")
	...
	log.Info("Light VSportChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light VSportChain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light VSportChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light VSportChain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light VSportChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light VSportChain message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the VSportChain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // VSportChain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
