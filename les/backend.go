...
// Package les implements the Light VSportChain Subprotocol.
package les
...
import (
	...
	"github.com/vsportchain/go-vsc/light"
	...
	rpc "github.com/vsportchain/go-vsc/rpc"
)

type LightVSportChain struct {
	config *eth.Config
...
func New(ctx *node.ServiceContext, config *eth.Config) (*LightVSportChain, error) {
...
// APIs returns the collection of RPC services the vsportchain package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightVSportChain) APIs() []rpc.API {
...
func (s *LightVSportChain) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightVSportChain) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightVSportChain) TxPool() *light.TxPool              { return s.txPool }
func (s *LightVSportChain) Engine() consensus.Engine           { return s.engine }
func (s *LightVSportChain) LesVersion() int                    { return int(s.protocolManager.SubProtocols[0].Version) }
func (s *LightVSportChain) Downloader() *downloader.Downloader { return s.protocolManager.downloader }
func (s *LightVSportChain) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (s *LightVSportChain) Protocols() []p2p.Protocol {
	return s.protocolManager.SubProtocols
}

// Start implements node.Service, starting all internal goroutines needed by the
// VSportChain protocol implementation.
func (s *LightVSportChain) Start(srvr *p2p.Server) error {
...
func (s *LightVSportChain) Stop() error {
	s.odr.Stop()
