...
import (
	...
	gvscmetrics "github.com/vsportchain/go-vsc/metrics"
	"github.com/vsportchain/go-vsc/metrics/influxdb"
...
func Setup(ctx *cli.Context) {
	if gvscmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gvscmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
