...
#
# The cluster contains a bootnode, a gvsc node and multiple swarm nodes, with
...
# static gvsc configuration
GVSC_IP="192.168.33.3"
GVSC_RPC_PORT="8545"
GVSC_RPC_URL="http://${GVSC_IP}:${GVSC_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gvsc and
# swarm nodes
start_bootnode() {
...
# start_gvsc_node starts a gvsc node with --datadir pointing at <base-dir>/gvsc
# and a single, unlocked account with password "gvsc"
start_gvsc_node() {
  local dir="${base_dir}/gvsc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gvsc account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GVSC_IP}"
    --rpcport   "${GVSC_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gvsc" "${GVSC_IP}" "$(which gvsc)" ${args[@]}
}
...
create_account() {
  ...
  gvsc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
