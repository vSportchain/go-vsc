## Go VSportChain Dashboard

The dashboard is a data visualizer integrated into gvsc, intended to collect and visualize useful information of an VSportChain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-vsc` repo), these need to be installed first:

```
$ (cd dashboard/assets && yarn install && yarn flow)
```

Normally the dashboard assets are bundled into Gvsc via `go-bindata` to avoid external dependencies. Rebuilding Gvsc after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ gvsc --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Gvsc, run `go generate`:
