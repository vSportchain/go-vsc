...
import (
	...
	"github.com/vsportchain/go-vsc/accounts/usbwallet/internal/trezor"
...
// or whether the VSportChain app was not started on it.
...
// VSportChain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.VSportChainAddress)
	if _, err := w.trezorExchange(&trezor.VSportChainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.VSportChainSignTx{
	...
	response := new(trezor.VSportChainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.VSportChainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
