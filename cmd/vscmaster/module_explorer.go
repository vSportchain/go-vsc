...
FROM vscmaster/explorer:latest

ADD vscstats.json /vscstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../eth-net-intelligence-api && pm2 start /vscstats.json)' >  explorer.sh && \
...
// explorerEthstats is the configuration file for the vscstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.vsportchain
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	vscstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(vscstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.vscstats[:strings.Index(config.vscstats, ":")],
		"Secret": config.vscstats[strings.Index(config.vscstats, ":")+1 : strings.Index(config.vscstats, "@")],
		"Host":   config.vscstats[strings.Index(config.vscstats, "@")+1:],
	})
	files[filepath.Join(workdir, "vscstats.json")] = vscstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.vsportchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		vscstats: infos.envvars["STATS"],
	}
	return stats, nil
}
