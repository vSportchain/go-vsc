...
// config contains all the configurations needed by vscmaster that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	vscstats  string   // Ethstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save vscmaster configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // VSportChain services known to be running on servers
...
// converts it to an VSportChain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by vscstats, which compares IPs textually, not by value.
