...
// nodeDockerfile is the Dockerfile required to run an VSportChain node.
var nodeDockerfile = `
FROM vsportchain/client-go:latest
...
{{end}}
RUN \
  echo 'gvsc --cache 512 init /genesis.json' > gvsc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.vsportchain/keystore/ && cp /signer.json /root/.vsportchain/keystore/' >> gvsc.sh && \{{end}}
	echo $'gvsc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --vscstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--vscbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gvsc.sh

ENTRYPOINT ["/bin/sh", "gvsc.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an VSportChain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.vsportchain{{if .Ethashdir}}
...
// deployNode deploys a new VSportChain node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.vscbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ethstats":  config.vscstats,
		"Etherbase": config.vscbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethstats":   config.vscstats[:strings.Index(config.vscstats, ":")],
		"Etherbase":  config.vscbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.vscstats,
	}
	if info.gasTarget > 0 {
		...
		if info.vscbase != "" {
			...
			report["Miner account"] = info.vscbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gvsc --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.vsportchain"],
		ethashdir:  infos.volumes["/root/.ethash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		vscstats:   infos.envvars["STATS_NAME"],
		vscbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
