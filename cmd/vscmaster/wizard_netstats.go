...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for vscstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["vscstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["vscstats"] = infos.Report()
		vscstats = infos.config
	}
