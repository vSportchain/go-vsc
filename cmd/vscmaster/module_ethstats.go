...
// vscstatsDockerfile is the Dockerfile required to build an vscstats backend
// and associated monitoring site.
var vscstatsDockerfile = `
FROM vscmaster/vscstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// vscstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an vscstats monitoring site.
var vscstatsComposefile = `
version: '2'
services:
  vscstats:
    build: .
    image: {{.Network}}/vscstats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new vscstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(vscstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(vscstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the vscstats service
	if nocache {
...
// vscstatsInfos is returned from an vscstats status check to allow reporting
// various configuration parameters.
type vscstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *vscstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an vscstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*vscstatsInfos, error) {
	// Inspect a possible vscstats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_vscstats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
