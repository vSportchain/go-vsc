...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"vscstats", "explorer", "wallet", "faucet"} {
	...
	// If we have vscstats running, ask whether to make the secret public or not
	if w.conf.vscstats != "" {
		fmt.Println()
		fmt.Println("Include vscstats secret on dashboard (y/n)? (default = yes)")
