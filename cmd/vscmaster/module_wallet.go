...
FROM vscmaster/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gvsc --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gvsc --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --vscstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/VscmasterNetworkID/{{.NetworkID}}/g' dist/js/etherwallet-master.js && \
	sed -i 's/VscmasterNetwork/{{.Network}}/g'     dist/js/etherwallet-master.js && \
	sed -i 's/VscmasterDenom/{{.Denom}}/g'         dist/js/etherwallet-master.js && \
	sed -i 's/VscmasterHost/{{.Host}}/g'           dist/js/etherwallet-master.js && \
	sed -i 's/VscmasterRPCPort/{{.RPCPort}}/g'     dist/js/etherwallet-master.js
