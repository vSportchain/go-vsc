...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.vscstats == "" {
		log.Error("No vscstats server configured")
		return
	}
	...
	if infos.vscstats == "" {
		...
		infos.vscstats = w.readString() + ":" + w.conf.vscstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.vscstats)
		infos.vscstats = w.readDefaultString(infos.vscstats) + ":" + w.conf.vscstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an vscbase to mine against
			fmt.Println()
			if infos.vscbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.vscbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.vscbase)
				infos.vscbase = w.readDefaultAddress(common.HexToAddress(infos.vscbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy VSportChain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
