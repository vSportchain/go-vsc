...
type vscstatsConfig struct {
	URL string `toml:",omitempty"`
}

type gvscConfig struct {
	Eth       eth.Config
	Shh       whisper.Config
	Node      node.Config
	Ethstats  vscstatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *gvscConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gvsc.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, gvscConfig) {
	// Load defaults.
	cfg := gvscConfig{
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the VSportChain Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
