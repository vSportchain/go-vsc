...
func TestAccountListEmpty(t *testing.T) {
	gvsc := runGvsc(t, "account", "list")
	gvsc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gvsc := runGvsc(t, "account", "list", "--datadir", datadir)
	defer gvsc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gvsc := runGvsc(t, "account", "new", "--lightkdf")
	defer gvsc.ExpectExit()
	gvsc.Expect(`
...
`)
	gvsc.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gvsc := runGvsc(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gvsc.ExpectExit()
	gvsc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gvsc.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gvsc.StderrText(), m) {
...
// https://github.com/vsportchain/go-vsc/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gvsc.SetTemplateFunc("keypath", func(file string) string {
