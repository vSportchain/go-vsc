...
import (
	...
	"github.com/vsportchain/go-vsc/p2p/discover"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the VSportChain protocol")
	statsFlag   = flag.String("vscstats", "", "Ethstats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for VSportChain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // VSportChain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an VSportChain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // VSportChain protocol stack
	client *ethclient.Client   // Client connection to the VSportChain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the VSportChain light client protocol
	...
	// Assemble the vscstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightVSportChain
			ctx.Service(&serv)
			return vscstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
...
// close terminates the VSportChain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the VSportChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/vsportchain/go-vsc/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and VSportChain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for VSportChain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No VSportChain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the VSportChain address and profile URL.
...
// returning the username, avatar URL and VSportChain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain VSportChain address,
