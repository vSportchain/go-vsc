...
func newTester(t *testing.T, confOverride func(*eth.Config)) *tester {
	...
	// Create a networkless protocol stack and start an VSportChain service within
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return eth.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register VSportChain protocol: %v", err)
	}
	...
	stack.Service(&vsportchain)

	return &tester{
		workspace: workspace,
		stack:     stack,
		vsportchain:  vsportchain,
		console:   console,
		input:     prompter,
		output:    printer,
	}
