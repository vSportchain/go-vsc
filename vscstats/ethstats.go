...
// Package vscstats implements the network stats reporting service.
package vscstats
...
import (
	...
	"github.com/vsportchain/go-vsc/common/mclock"
...
// Service implements an VSportChain netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *eth.VSportChain      // Full VSportChain service if monitoring a full node
	les    *les.LightVSportChain // Light VSportChain service if monitoring a light node
...
func New(url string, ethServ *eth.VSportChain, lesServ *les.LightVSportChain) (*Service, error) {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the vscstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to vscstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to vscstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to vscstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to vscstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to vscstats")
